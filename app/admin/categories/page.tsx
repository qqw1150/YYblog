...
export default function CategoriesManagement() {
  ...
  const [formErrors, setFormErrors] = useState<{ name?: string; slug?: string; description?: string }>({});
  ...
  const loadCategories = async () => {
    try {
      ...
      // 获取分类统计信息（包含文章数量）
      const { data: categoryStats, error: statsError } = await getCategoryStats();
      
      if (statsError) {
        console.error('获取分类统计信息失败:', statsError);
      ...
      if (categoryStats) {
        ...
        if (searchTerm) {
          filteredCategories = categoryStats.filter(category => 
   ...
   * 处理表单数据变化
   */
  const handleFormChange = (field: keyof CategoryCreate, value: string) => {
  ...
  const validateForm = (): boolean => {
    ...
    if (!formData.slug.trim()) {
      errors.slug = '分类别名不能为空';
    } else if (!/^[a-z0-9\-]+$/.test(formData.slug)) {
      errors.slug = '分类别名只能包含小写字母、数字和连字符';
    } else if (formData.slug.length > 50) {
      errors.slug = '分类别名不能超过50个字符';
    }
  ...
  const handleDeleteCategory = async (id: string, name: string) => {
    if (confirm(`确定要删除分类"${name}"吗？此操作无法撤销。`)) {
      try {
        setIsLoading(true);
      ...
      <div className="bg-white rounded-lg shadow overflow-hidden">
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center p-8 text-gray-500">
            ...
            <p className="text-sm">
              {searchTerm ? '请尝试其他搜索关键词' : '点击"新建分类"开始创建'}
            </p>
          </div>
        ) : (
          <>
                    ...
                    <tr key={category.id} className="hover:bg-gray-50">
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center">
                          <FolderTree size={16} className="text-green-500 mr-2" />
